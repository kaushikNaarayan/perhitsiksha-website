-- Create page_views table for cumulative page view counting
...
DROP POLICY IF EXISTS "Public read access" ON page_views;
CREATE POLICY "Public read access" ON page_views
FOR SELECT USING (true);

-- Allow public increment via RPC
DROP POLICY IF EXISTS "Public increment via RPC" ON page_views;
CREATE POLICY "Public increment via RPC" ON page_views
FOR INSERT WITH CHECK (true);

DROP POLICY IF EXISTS "Public update via RPC" ON page_views;
