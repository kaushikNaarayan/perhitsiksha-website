...
const YouTubeEmbed: React.FC<YouTubeEmbedProps> = ({
  ...
  const handleImageError = () => {
    if (currentThumbnailIndex < thumbnailUrls.length - 1) {
      console.warn(
        `YouTube thumbnail failed for video ${videoId}, trying fallback ${currentThumbnailIndex + 1}`
      );
      setCurrentThumbnailIndex(currentThumbnailIndex + 1);
    } else {
      console.error(
        `All YouTube thumbnails failed for video ${videoId} - showing generic fallback`
      );
