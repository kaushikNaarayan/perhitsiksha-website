...
const EnhancedCarousel: React.FC<EnhancedCarouselProps> = ({ endorsements }) => {
  ...
  const nextSlide = () => {
    const newIndex = currentIndex === endorsements.length - 1 ? 0 : currentIndex + 1;
    goToSlide(newIndex);
  };

  const prevSlide = () => {
    const newIndex = currentIndex === 0 ? endorsements.length - 1 : currentIndex - 1;
    goToSlide(newIndex);
  };
    ...
    <div className="relative">
      ...
      <div className="md:hidden">
        ...
        >
          ...
          {endorsements.map((celebrity) => (
            <div
              key={celebrity.id}
              className="flex-none w-56 snap-center"
            >
              {/* Celebrity Info */}
      ...
      <div className="hidden md:block">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {endorsements.slice(currentIndex, currentIndex + (isDesktop ? 4 : 2)).map((celebrity) => (
        ...
        <div className="flex justify-center mt-6 space-x-2">
          {Array.from({ length: Math.ceil(endorsements.length / 4) }).map((_, index) => (
            <button
              key={index}
